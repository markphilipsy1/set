...
<body>
	<nav class="navbar navbar-expand-sm navbar-dark fixed-top" style="background-color: #287722;">
  	...
  	<div class="container">
  		<p class="h5">
  		<?php 
  			$evalnumsql = mysqli_query($connect, "SELECT COUNT(*) as evalnum FROM tbl_eval a INNER JOIN tbl_prof b ON a.prof_id = b.prof_id INNER JOIN tbl_period c ON a.period = c.id WHERE b.prof_campus = '$campus' AND c.id = (SELECT id FROM tbl_period WHERE active = 1)");
  			$evalnum = mysqli_fetch_assoc($evalnumsql);

  			echo $campus."<small style='float: right;'> Number of Evaluations Submitted: ".$evalnum['evalnum']."</small>"; ?></p>
  		<hr>
  		...
  		<?php 
  			if (!isset($_POST['changedept']) || $_POST['dept'] == 'all') {
	  			$sql = "SELECT a.prof_id, CONCAT(a.prof_lname , ', ', a.prof_fname) AS prof_name, a.prof_col, a.prof_dept FROM tbl_prof a INNER JOIN enrollscheduletbl b ON a.prof_id = b.instructor WHERE prof_campus = '$campus' AND b.schoolyear = (SELECT year FROM tbl_period WHERE active = 1) AND b.semester = (SELECT semester FROM tbl_period WHERE active = 1) ORDER BY prof_lname";
	  		$res = mysqli_query($connect, $sql);	
	  		}
			else {
				$dept = $_POST['dept'];

				$sql = "SELECT a.prof_id, CONCAT(a.prof_lname , ', ', a.prof_fname) AS prof_name, a.prof_col, a.prof_dept FROM tbl_prof a INNER JOIN enrollscheduletbl b ON a.prof_id = b.instructor WHERE prof_campus = '$campus' AND prof_dept = '$dept' AND b.schoolyear = (SELECT year FROM tbl_period WHERE active = 1) AND b.semester = (SELECT semester FROM tbl_period WHERE active = 1) ORDER BY prof_lname";
			  		$res = mysqli_query($connect, $sql);
			}
	  		...
	  		<?php 
	  		 ...
	  		 ?>
	  		 <br>
	  		 <a href="report_campus.php?prof_campus=<?php echo $campus ?>"><button class="btn btn-light" style="float: left;"><span class="fas fa-file-alt"></span> Summary of Reports</button></a>
	  		 <br>
	  		 <br>
  	</div>
  	<div class="container">
