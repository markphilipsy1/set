...
<body>
	<nav class="navbar navbar-expand-sm navbar-dark fixed-top" style="background-color: #287722;">
  	...
  	<div class="container">
  		...
  		<?php 
			$sql = mysqli_query($connect, "SELECT a.prof_id, COUNT(a.prof_id) as samp_size, CONCAT(b.prof_fname,' ',b.prof_lname) as prof_name FROM ".$_SESSION['semyr']." a INNER JOIN tbl_prof b ON a.prof_id = b.prof_id WHERE b.prof_col = '$college' GROUP BY a.prof_id");
			...
			function thescore($prof){
				...
				for ($l=0; $l < count($range)-1; $l++) {
					...
					$sql = mysqli_query($connect, "SELECT COUNT(a.prof_id) as samp_size, CONCAT(b.prof_fname,' ',b.prof_lname) as prof_name, SUM($scores) as total FROM ".$_SESSION['semyr']." a INNER JOIN tbl_prof b ON a.prof_id = b.prof_id WHERE b.prof_col = '$college' AND a.prof_id = $prof GROUP BY a.prof_id");
					$row_cnt = mysqli_num_rows($sql);
					$sum = array();
					while ($row = mysqli_fetch_assoc($sql)) {
