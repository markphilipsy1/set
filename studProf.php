  				...
  				<tbody>
  					<?php 
			  			while ($sub = mysqli_fetch_array($res1)) {
			  				if ($sub['instructor'] != '-') {
				  				$sql_check = "SELECT * FROM tbl_eval WHERE studentnumber = ".$_SESSION['studentnumber']." AND prof_id = ".$sub['instructor']." AND schedcode = ".$sub['schedcode']." AND period = (SELECT MAX(id) FROM tbl_period)";
			  				...
			  				?>
			  				<td class="text-center"><a href="evalForm_main.php?prof_id=<?php echo $sub['instructor'];?>&schedcode=<?php echo $sub['schedcode'] ?>"><button class="btn btn-light" id="<?php echo $sub['instructor'] ?>" name="<?php echo $sub['schedcode'] ?>"><span class="fa fa-edit"></span></button></a></td>
			  		<?php
				  			if ($row_check) {
				  				echo "<script>
				  					document.getElementById(".$sub['schedcode'].").setAttribute('disabled','');
				  					document.getElementById(".$sub['schedcode'].").innerHTML = 'Evaluated';
				  					document.getElementById(".$sub['schedcode'].").setAttribute('class','btn btn-light text-success');
				  					</script>";
				  			}
			  				echo "</tr>";
			  			}
